  METHOD m_get_pr_error_data.
    ...
    DATA: lt_matnr    TYPE /iwbep/t_cod_select_options,
          ...
          lt_msgtyp   TYPE /iwbep/t_cod_select_options,
...
*-- Get the PR item details from the method m_get_eban_data
    me->m_get_error_from_spool( IMPORTING et_eban = gt_eban ).
...
*-- Retrieve the Filter selection details from the UI application
      IF  it_filter_select_options IS NOT INITIAL.
        ...
        lt_msgtyp = VALUE #( it_filter_select_options[ property = gc_msgtyp ]-select_options OPTIONAL ). "#EC CI_STDSEQ
        DELETE lt_msgtyp WHERE low IS INITIAL.
*-- Delete the filter details from EBAN table
        DELETE gt_eban WHERE banfn NOT IN lt_prnumber OR bnfpo NOT IN lt_pritem OR msgtyp NOT IN lt_msgtyp
        OR message NOT IN lt_error.                     "#EC CI_SORTSEQ
      ENDIF.
      DATA(lt_pr) = gt_eban.
          ...
          et_eban = VALUE #(
                             FOR gs_eban IN gt_eban
                                     ( banfn = gs_eban-banfn
                                       ...
                                       messagetype = gs_eban-msgtyp
                                       message   = gs_eban-message
           ) ) ##WARN_OK.
          DELETE et_eban WHERE matnr NOT IN lt_matnr OR werks NOT IN lt_plant OR flief NOT IN lt_vendor OR ekgrp NOT IN lt_purchorg.
        ELSE.
*-- Pass the final details to the exporting table
          IF lt_pr IS NOT INITIAL AND lt_matnr IS INITIAL AND lt_plant IS INITIAL AND lt_vendor IS INITIAL AND lt_purchorg IS INITIAL.
            et_eban = VALUE #(
                               FOR gs_eban IN gt_eban
                               ( banfn = gs_eban-banfn
                                 bnfpo = gs_eban-bnfpo
